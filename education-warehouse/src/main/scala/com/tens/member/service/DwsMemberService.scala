...
import com.tens.member.bean.{DwsMember, DwsMember_Result, MemberZipper, MemberZipperResult}
import com.tens.member.dao.DwdMemberDao
import org.apache.spark.sql.{Dataset, SaveMode, SparkSession}

object DwsMemberService {
  def importMemberApi(sparkSession: SparkSession, dt: String) = {
    import sparkSession.implicits._ //导入隐式转换
    //获取主表用户表
    val dwdMember = DwdMemberDao.getDwdMember(sparkSession).where(s"""dt=${dt}""")
    //获取
    ...
    val resultData = result.groupByKey(item => item.uid + "_" + item.dn)
      .mapGroups { case (key, items) =>
        val keys = key.split("_")
        val uid = Integer.parseInt(keys(0))
        val dn = keys(1)
        val dwsMembers = items.toList
        ...
        val vip_last_modify_time = dwsMembers.map(_.vip_last_modify_time).last
  ...
  def importMember(sparkSession: SparkSession, time: String) = {
    import sparkSession.implicits._ //冥界召唤
    //查询全量数据 刷新到宽表
    sparkSession.sql(s"""select
                        ...
                        |                b.appregurl,
                        |                b.bdp_uuid,
                        |                b.createtime as reg_createtime,b.domain,
                        |                b.isranreg,
                        |                b.regsource,
                        |                b.regsourcename,
                        |                c.adname,
    ...
    val dayResult = sparkSession.sql(
      s"""
         |SELECT
         |t1.uid ,
         |SUM(CAST (t1.paymoney as decimal(10,4))) as paymoney ,
         |MAX(t2.vip_level ) as vip_level ,
         |from_unixtime(unix_timestamp('$time','yyyyMMdd'),'yyyy-MM-dd') as start_time,
         |'9999-12-31' as end_time,
         |FIRST (t1.dn ) as dn
         |FROM dwd.dwd_pcentermempaymoney t1
         |join dwd.dwd_vip_level t2
    ...
    dayResult.union(historyResult)
      ...
      .mapGroups{
        case (key, iters) =>
        val list = iters.toList.sortBy(_.start_time)
        if(list.size > 1 && "9999-99-99".equals(list(list.size -2 ).end_time)) {
          //获取历史数据的最后一条数据
          val oldLastModel = list(list.size - 2)
          //获取当前时间的最后一条数据
          val lastModel = list(list.size - 1)

          oldLastModel.end_time = lastModel.end_time
          lastModel.paymoney = (BigDecimal(lastModel.paymoney) + BigDecimal(oldLastModel.paymoney)).toString()
        }
