...
import com.tens.qz.bean.{DwdQzPaperView, DwdQzPoint, DwdQzQuestion}
...
object EtlDataService {
  ...
  def etlQzWebsite(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/tens/ods/QzWebsite.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
  ...
  def etlQzSiteCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzSiteCourse.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzQuestionType(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzQuestionType.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzQuestion.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzPointQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzPointQuestion.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzPoint(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzPoint.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzPaperView(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzPaperView.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzPaper(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzPaper.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzMemberPaperQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzMemberPaperQuestion.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzMajor(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzMajor.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzCourseEdusubject(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzCourseEduSubject.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzCourse.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzChapterList(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzChapterList.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzChapter(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/tens/ods/QzChapter.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
        partitions.map(item => {
          val jsonObject = JSON.parseObject(item)
  ...
  def etlQzCenterPaper(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzCenterPaper.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
      .mapPartitions(partitions => {
      partitions.map(item => {
        val jsonObject = JSON.parseObject(item)
  ...
  def etlQzCenter(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzCenter.log")
      .filter(item => JSON.parseObject(item).isInstanceOf[JSONObject])
  ...
  def etlQzBusiness(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/tens/ods/QzBusiness.log")
