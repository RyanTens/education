...
object DwsQzService {
  def saveDwsUserPaperDetail(sparkSession: SparkSession, dt: String): Unit = {
    ...
    val dwdQzMemberPaperQuestion = UserPaperDetailDao
      .getDwdQzMemberPaperQuestion(sparkSession, dt)
      .drop("paperid")
      .withColumnRenamed("question_answer", "user_question_answer")
    val dwsQzChapter =
      UserPaperDetailDao.getDwsQzChapter(sparkSession, dt).drop("courseid")
    val dwsQzCourse = UserPaperDetailDao
      .getDwsQzCourse(sparkSession, dt)
    ...
    val dwsQzPaper =
      UserPaperDetailDao.getDwsQzPaper(sparkSession, dt).drop("courseid")
    val dwsQzQuestion = UserPaperDetailDao.getDwsQzQuestion(sparkSession, dt)
    dwdQzMemberPaperQuestion
      .join(dwsQzCourse, Seq("sitecourseid", "dn"))
  ...
  def saveDwsQzQuestionTpe(sparkSession: SparkSession, dt: String) = {
    ...
    val result = dwdQzQuestion
      .join(dwdQzQuestionType, Seq("questypeid", "dn"))
      .select(
        "questionid",
        "parentid",
        "questypeid",
    ...
    result
      ...
      .write
      .mode(SaveMode.Append)
      .insertInto("dws.dws_qz_question")
  }
  ...
  def saveDwsQzCourse(sparkSession: SparkSession, dt: String) = {
    ...
    val dwdQzCourseEdusubject =
      QzCourseDao.getDwdQzCourseEduSubject(sparkSession, dt)
    val result = dwdQzSiteCourse
  ...
  def saveDwsQzChapter(sparkSession: SparkSession, dt: String) = {
    ...
    val dwdQzPointQuestion =
      QzChapterDao.getDwdQzPointQuestion(sparkSession, dt)
    val result = dwdQzChapter
